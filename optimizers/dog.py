...
def build_dog(params, args):
    """
    Adapter to create DoG or LDoG from your refactored CLI.
    It reuses existing flags when possible and adds robust fallbacks:

    - reps_rel: uses --reps-rel if present, else falls back to --alpha, else 1e-6
    - lr: uses --lr (default 1.0)
    - weight_decay: uses --weight-decay
    - eps: uses --eps
    - init_eta: uses --init-eta if present
    - layerwise toggle: --dog-layerwise (bool) selects LDoG
    """
    reps_rel = getattr(args, "reps_rel", None)
    if reps_rel is None:
        # Backward compatibility: many of your runs used --alpha for DoG-like step knobs
        reps_rel = getattr(args, "alpha", 1e-6)

    lr = getattr(args, "lr", 1.0)
    weight_decay = getattr(args, "weight_decay", 0.0)
    eps = getattr(args, "eps", 1e-8)
    init_eta = getattr(args, "init_eta", None)

    use_layerwise = bool(getattr(args, "dog_layerwise", False))

    if use_layerwise:
        # LDoG: layer-wise rbar/G, same hyperparams
        return LDoG(
            params,
            reps_rel=reps_rel,
            lr=lr,
        ...
        # DoG: global rbar/G across params
        return DoG(
            params,
            reps_rel=reps_rel,
            lr=lr,
